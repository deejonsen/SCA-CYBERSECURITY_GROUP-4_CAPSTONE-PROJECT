...
1.  **Password Policy Review:** A password policy is a set of rules that define how users must create and manage their passwords. It is different from a password policy review which is the process of evaluating those rules to ensure they are strong, relevant, and not creating security gaps.
...
Password reviews matter because they are one of the simplest ways to prevent credential stuffing and brute-force attacks.

Poor password policies can lead to:
...
- **Password Length:** This matters because short passwords are easily subjected to brute-force attacks.
- Good Practice: minimum of 12–16 characters
- Weak Policy: minimum of 6 characters
- **Complexity Requirements:** Always ensure your password contains the following;
- Uppercase letters (A–Z)
- Lowercase letters (a–z)
- Numbers (0–9)
- Special characters (!@#$)
- **Account Lockout Policy:** Account should be locked after 3 failed attempt. This will stop brute-force attacks. No lockout policy makes an attacker try unlimited guesses.
- **Multi-factor Authentication:** An MFA acts as the most critical defense when a password is stolen or weak. In an IAM context, MFA ensures that a single compromised credential does not grant an attacker full access to cloud infrastructure.
- **User Awareness & Behavior:** An account user should not share passwords, write them on paper or use the same password everywhere. Password review must consider human behavior.
...
- Collect Existing Policy: from system admin or documentation
- Compare with Best Practices: use standards like NIST guidelines.
- Identify Weaknesses: Ask questions like,
- Are passwords too short?
- Is MFA missing?
- Is reuse allowed?
- Assess Risk: what happens if one account is compromised?
...
1.  **Role Assignments:** Role assignment is the process of linking a user, group, or identity to a defined role that specifies what actions they can perform within a system. It enforces access control by granting permissions based on responsibilities rather than individual accounts.
...
1.  Identified accounts with and without MFA enabled.
2.  Tested enforcement by attempting logins with MFA enabled & disabled.
3.  Documented gaps, especially for privileged accounts.

**Least Privilege & Role Assignments**

1.  Reviewed IAM roles and group memberships.
2.  Compared assigned privileges with actual job functions.
3.  Checked for excessive or misaligned privileges.

**Account & Privilege Escalation Risks**

1.  Identified dormant, shared, or service accounts with broad rights.
2.  Simulated privilege escalation scenarios.
3.  Documented risks and potential attack paths.
...
# **Standards Comparison****NIST SP 800-63B**
...
| **Password Policy Review** | Weak password length | High | Critical |
| **MFA Enforcement** | Token secret missing or time not synced | High | Enforce MFA for all privileged accounts, ensure clock sync |
...
| **Accounts Audit** | Old accounts left enabled | High | Disable or delete unused accounts immediately |
| **Privilege Escalation Risks** | Misconfigured roles allow admin rights | Medium | Monitor role changes, enforce MFA, audit privileges regularly |

**Recommendations**

1.  Enforce stronger password policies.
2.  Increase password history size (≥5).
3.  Require ≥3-character classes for stronger complexity.
4.  Mandate MFA for all privileged accounts.
5.  Implement least privilege principles.
6.  Regularly audit and clean up IAM roles.
7.  Disable stale accounts immediately.
8.  Tighten sudo rules to least privilege
...
**MFA Best Practices (NIST):** https://pages.nist.gov/800-63-3/sp800-63b.html

**Evidences:** [https://github.com/deejonsen/SCA-CYBERSECURITY_GROUP-4_CAPSTONE-PROJECT/Screenshots](https://github.com/deejonsen/SCA-CYBERSECURITY_GROUP-4_CAPSTONE-PROJECT/ScreenshotsCT)

**Capstone Project Hands-on Lab Work:** https://drive.google.com/drive/folders/125zVhusEockm8KyDLaec9qsJ29lFAnR4?usp=drive_link